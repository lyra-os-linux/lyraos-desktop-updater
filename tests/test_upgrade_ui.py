...
class UpgradeUiContractTests(unittest.TestCase):
    ...
    def test_planning_uses_the_service_because_zypper_dry_run_requires_root(self) -> None:
        rust = (UPGRADE / "src-tauri/src/main.rs").read_text(encoding="utf-8")
        app = (UPGRADE / "ui/app.js").read_text(encoding="utf-8")
        self.assertNotIn("async fn plan_update", rust)
        self.assertNotIn("spawn_blocking(plan_update_with_cached_metadata)", rust)
        self.assertNotIn('invoke("plan_update"', app)
        self.assertIn('request("PlanUpdate")', app)
        self.assertIn('request("Start"', app)
        self.assertLess(app.index('request("PlanUpdate")'), app.index('request("Start"'))
